...
namespace BitcoinLite.Tests
{
	...
	public class Bip32Tests
	{
		...
		{
;			var extkey = HdKey.Parse("xprv9s21ZrQH143K3Z9EwCXrA5VbypnvWGiE9z22S1cLLPi7r8DVUkTabBvMjeirS8KCyppw24KoD4sFmja8UDU4VL32SBdip78LY6sz3X2GPju");
		...
		public void TestVectors(string entropyStr, string pub, string prv, uint child)
		{
			...
			Assert.AreEqual(prv, key.ToString(Network.Main));
			Assert.AreEqual(pub, pubkey.ToString(Network.Main));

			// Derive new keys
			var keyNew = key.Derive(child);
			var pubkeyNew = keyNew.Neuter;
			if ((child & 0x80000000) == 0)
			{
				// Compare with public derivation
				var pubkeyNew2 = pubkey.Derive(child);
				Assert.AreEqual(pubkeyNew, pubkeyNew2);
			}
		}
	}
}

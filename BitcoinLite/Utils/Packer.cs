...
namespace BitcoinLite.Utils
{
	public abstract class Packer
	{
		...
		public uint160 ToUInt160(byte[] value, int offset)
		{
			var buf = new byte[uint160.Size];
			CopyBytes(value, offset, buf, 0, buf.Length);
			return new uint160(buf);
		}

		public uint256 ToUInt256(byte[] value, int offset)
		{
			var buf = new byte[uint256.Size];
			CopyBytes(value, offset, buf, 0, buf.Length);
		...
		{
			return GetBytes(value.ToByteArray(), uint160.Size);
		}

		public byte[] GetBytes(uint256 value)
		{
			return GetBytes(value.ToByteArray(), uint256.Size);
		}
		...
		static bool PackOne(PackContext b, object oarg)
		{
			...
			switch (b.description[b.i])
			{
				...
				case 'C':
					...
					break;
				case 'X':
					b.Add(b.conv.GetBytes(((IBinarySerializable)oarg).ToByteArray()));
					break;
				case 'A':
